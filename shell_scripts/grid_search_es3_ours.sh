...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment_runs/es3_grid_search}"
PATHWAY_FILE="${PATHWAY_FILE:-/raid/yangpeng_lab/c12212609/PRESAGE/data/topic_embed/ours_ess.txt}"
  ...
  for wd in 1e-15 1e-6; do
    for pool in gat vector; do
      for patience in 10 30; do
        run_idx=$((run_idx + 1))
        tag="es3_gs_${run_idx}"
        echo "[es3 ${run_idx}] lr=${lr} wd=${wd} pool=${pool} patience=${patience}"
