...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment_runs/gw1_grid_search}"
PATHWAY_FILE="${PATHWAY_FILE:-/raid/yangpeng_lab/c12212609/PRESAGE/data/topic_embed/all_gw.txt}"

# Anchor config: lr=1.35e-3, wd=1e-15, batch=256, item_hidden=1024, pathway_hidden=256, pathway_nlayers=3
mkdir -p "$OUTPUT_ROOT"
cd "$NOTEBOOK_DIR"
...
for temp in 0.08 0.12; do
  for patience in 10 30; do
        run_idx=$((run_idx + 1))
        tag="gw1_gs_${run_idx}"
        echo "[gw1_gs ${run_idx}] lr=${lr} wd=${wd} temp=${temp} patience=${patience}"
        CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" run_presage_experiment.py               --dataset "$DATASET"               --seed "$SEED_NAME"               --pathway_files "$PATHWAY_FILE"               --use_training_gex_embeddings \
      --tag "$tag"               --output_root "$OUTPUT_ROOT"               --bootstrap_iters "$BOOTSTRAP_ITERS"               --patience "$patience"     --enhance_added_embeddings               --optimizer AdamW               --scheduler cosine               --warmup_epochs 5               --min_lr_ratio 0.05               --gradient_clip_val 0.5               --batch_size 256               --lr "$lr"               --weight_decay "$wd"               --max_epochs 10000               --precision 32               --pathway_weight_type gat               --pool_nlayers 2               --softmax_temperature "$temp"               --gat_weight 0.85               --item_hidden_size 1024               --item_nlayers 0               --pathway_item_hidden_size 256               --pathway_item_nlayers 3               --item_dropout 0.08               --pathway_dropout 0.08               --source_dropout 0.10               --pathway_layer_norm
      done
    done
  done
done
...
"item_hidden_size": cfg.get("model", {}).get("item_hidden_size"),
