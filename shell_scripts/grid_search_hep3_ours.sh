...
for pool in gat vector; do
  for patience in 10 30; do
      ...
      --tag "$tag"               --output_root "$OUTPUT_ROOT"               --bootstrap_iters "$BOOTSTRAP_ITERS"               --patience "$patience"     --enhance_added_embeddings               --optimizer AdamW               --scheduler cosine               --warmup_epochs 5               --min_lr_ratio 0.05               --gradient_clip_val 0.5               --batch_size 16               --lr "$lr"               --weight_decay "$wd"               --max_epochs 10000               --precision 32               --pathway_weight_type "$pool"               --pool_nlayers 2               --softmax_temperature 0.10               --gat_weight 0.80               --item_hidden_size 512               --item_nlayers 0               --pathway_item_hidden_size 128               --pathway_item_nlayers 2               --item_dropout 0.10               --pathway_dropout 0.08               --source_dropout 0.05               --pathway_layer_norm
      done
    done
  done
done
