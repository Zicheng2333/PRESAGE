...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment_runs/es5_grid_search}"
  ...
  for n_nmf in 64 128; do
    for pathway_hidden in 64 128; do
      for patience in 10 30; do
        ((run_idx += 1))
        tag="es5_gs_${run_idx}"
        echo "[es5_gs ${run_idx}] lr=${lr} n_nmf=${n_nmf} pathway_hidden=${pathway_hidden} patience=${patience}"
