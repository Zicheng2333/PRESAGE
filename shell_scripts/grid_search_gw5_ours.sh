...
for pathway_hidden in 256 384; do
  for patience in 10 30; do
        ...
        CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON_BIN" run_presage_experiment.py               --dataset "$DATASET"               --seed "$SEED_NAME"               --pathway_files "$PATHWAY_FILE"               --tag "$tag"               --output_root "$OUTPUT_ROOT"               --bootstrap_iters "$BOOTSTRAP_ITERS"               --patience "$patience"     --enhance_added_embeddings               --optimizer AdamW               --scheduler cosine               --warmup_epochs 5               --min_lr_ratio 0.05               --gradient_clip_val 0.5               --batch_size 256               --lr "$lr"               --weight_decay 1e-6               --n_nmf_embedding "$n_nmf"               --max_epochs 10000               --precision 32               --pathway_weight_type gat               --pool_nlayers 2               --softmax_temperature 0.10               --gat_weight 0.85               --item_hidden_size 1024               --item_nlayers 0               --pathway_item_hidden_size "$pathway_hidden"               --pathway_item_nlayers 3               --item_dropout 0.10               --pathway_dropout 0.08               --source_dropout 0.10               --pathway_layer_norm
      done
    done
  done
done
...
"pathway_item_nlayers": cfg.get("model", {}).get("pathway_item_nlayers"),
