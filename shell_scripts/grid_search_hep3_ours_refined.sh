...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment_runs/hep3_grid_search_refined}"
...
for backbone in new old; do
  ...
  for profile in legacy_like cosine_light; do
    ...
    for lr in 6.5e-4 8.2e-4 1.0e-3; do
      for pool in gat mean; do
        for n_nmf in 64 128; do
          ((run_idx += 1))
          tag="hep3_ref_${backbone}_${profile}_${run_idx}"
          echo "[hep3_ref ${run_idx}] backbone=${backbone} profile=${profile} lr=${lr} pool=${pool} n_nmf=${n_nmf}"
