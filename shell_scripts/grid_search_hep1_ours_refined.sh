...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment_runs/hep1_grid_search_refined}"
...
for backbone in new old; do
  ...
  for profile in legacy_like cosine_light; do
    ...
    for lr in 4.5e-4 5.5e-4 8.2e-4; do
      for temp in 0.10 0.12 0.14; do
        ((run_idx += 1))
        tag="hep1_ref_${backbone}_${profile}_${run_idx}"
        echo "[hep1_ref ${run_idx}] backbone=${backbone} profile=${profile} lr=${lr} temp=${temp}"
